		...
		<div class="kt-portlet__head-toolbar">
			<div class="kt-portlet__head-wrapper">
				<div class="kt-portlet__head-actions">
					...
					<a href="{{ route('karyawan.create') }}" class="btn btn-brand btn-elevate btn-icon-sm la la-plus" >
					</a>
				</div>
			</div>
		</div>
...
<script>
		KTUtil.ready(function() {
			...
			var url = "{{ route('karyawan.getdata') }}";
			var targetRender = {
				'0':{'type' : '', 'orderable': false },
				'-1':{'type' : 'actions','orderable': false}
			};
			var filter = {
				'NIK':{'name':'karyawan_nik','type':'text'},
				'Nama':{'name':'karyawan_fullname','type':'text'},
				'Email':{'name':'karyawan_email','type':'text'},
				'Agama':{'name':'karyawan_agama','type':'text'},
				'Jabatan':{'name':'jabatan_nama','type':'text'},
				'Role':{'name':'role_name','type':'text'},
				'Tipe':{'name':'karyawan_tipe','type':'select','data':{!!$select2!!}},
			};
			var order   = [[0, "asc"]];
