@extends('template.layout')

@section('app_title',env('APP_NAME'))

@section('module_title',$module_title)

@section('container')
    <!-- begin:: Content -->
    <div class="kt-container--fluid  kt-grid__item kt-grid__item--fluid" id="my-vue">
        <div class="row">
            <div class="col-lg-12">
                
                <h1>@{{val.title}}</h1>
                <div class="form-group" row>
                    <validation-observer v-slot="{ passes }" >
                        {{ csrf_field() }}
                <!--begin::Portlet-->
                <div class="kt-portlet">
                    <div class="kt-portlet__head">
                        <div class="kt-portlet__head-label">
                            <h3 class="kt-portlet__head-title">
                                Mahasiswa Form
                            </h3>
                        </div>
                    </div>

                    <!--begin::Form-->
                    <form method="post" action="{{ route('mahasiswa.update', ['id' => $mahasiswa[0]->mahasiswa_id]) }}">
                        {{ csrf_field() }}
                        <div class="kt-portlet__body">
                            <div class="kt-section kt-section--first">
                                <div class="col-md-12 form-warning" style="display:none">
                                        @include('template.alert')
                                    </div>
                                    <div class="form-group">
                                        <validation-provider name="name" rules="required">
                                            <div slot-scope="{ errors }">
                                                <label>Nama Mahasiswa</label>
                                                <input type="text" name="name" v-model="form.name" value="{{@$mahasiswa[0]->mahasiswa_id}}" class="form-control">
                                                <p>@{{ errors[0] }}</p>
                                            </div>
                                        </validation-provider>
                                    </div>
                                <div class="form-group">
                                    <label for="jk">Jenis Kelamin</label>
                                    <select class="form-control" id="jk" readonly>
                                        <option>Laki - laki</option>
                                        <option>Perempuan</option>
                                    </select>
                            </div>
                        </div>
                        <div class="kt-form__actions">
                            <a href="/master/mahasiswa" class="btn btn-secondary core-pjax">Kembali</a>
                        </div>
                    </form>
                </div>
                </validation-observer>
                </div>
            </div>
        </div>
    </div>
@stop
@section("script")
    <script type="text/javascript">
        $( document ).ready(function() {
            CoreFormControls.init();
        }); 
    </script>

  @stop